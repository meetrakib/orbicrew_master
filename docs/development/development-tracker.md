...
### Context
With Phase 2.1 done, four Phase 2 slices were unblocked (2.2, 2.3, 2.5, 2.7). Asked the user which to build next via `AskUserQuestion`; they picked **2.3: BYO key management**, framing the business model directly: tenants either use the platform's pooled keys (future: usage capped by subscribed plan) or bring their own key (lower price, tenant's own provider bill, tenant responsible for their own cost).

`tech/13_byo_provider_architecture.md` describes 2.3's target as a full self-hosted LiteLLM gateway with per-tenant virtual keys — but that same doc's §0 (added the day Phase 1.8 shipped) already flags that Phase 1.8 chose a lighter in-app provider registry over that gateway for the same "no infra to operate for zero benefit yet" reason. Asked the user directly via a second `AskUserQuestion`: confirmed an **in-app extension** of the existing registry (no new infrastructure) over standing up LiteLLM now, and confirmed scope as **direct platform keys for the four providers the platform itself already knows how to call** — Anthropic, OpenAI, OpenRouter, DeepInfra (Pattern A + B merged into one catalog, since OpenRouter/DeepInfra already have first-class entries in `model_providers.py`). Pattern C (self-hosted/custom endpoint) explicitly deferred — the user noted it's more likely to become a *platform-side* cheaper-hosting option later than a tenant-facing field now.

A full plan (`EnterPlanMode`/`ExitPlanMode`) was written and approved before implementation.

### Done

**`repos/orbicrew-api`:**
- New `migrations/0008_byo_provider_keys.sql`: `byo_provider_keys` table (`tenant_id`, `provider` check-constrained to the four known providers, `encrypted_key bytea`, `key_last4`, `label`, `is_active`, `is_valid`, `last_validated_at`) — deliberately a **new** table, not an extension of the existing `api_keys` table from Phase 2.1: `api_keys.hashed_key` is a one-way SHA-256 digest for authenticating requests *to our own API* (channel adapters), structurally unable to hold a value that must be decrypted back into a usable third-party secret. `api_keys.is_byo` (added in 2.1, never used) is an unrelated leftover, left alone. A partial unique index (`where is_active`) enforces at most one active BYO provider per tenant, mirroring Phase 1.8's own single-global-switch shape but per-tenant. RLS + `orbicrew_app` grant follow the same pattern as every other tenant-scoped table since 2.1.
- New `src/orbicrew_api/crypto.py`: `encrypt_secret`/`decrypt_secret` via `cryptography`'s `Fernet`, keyed by a SHA-256 hash of new `settings.byo_key_encryption_secret` — the same pragmatic single-master-key approach `jwt_secret` already uses elsewhere in this codebase, not a KMS-backed envelope (documented as the future upgrade once this protects real customer keys at scale).
- `model_providers.py` gains `validate_provider_key()` (a cheap `.models.list()` probe on both the `anthropic` and `openai` SDKs — no completion tokens spent) and `complete()` gains an optional `api_key_override` param; `resolve_model()` gains `allow_manual_override` (defaults `True`) so a tenant's BYO call can never inherit the platform admin's own `MODEL_PROVIDER_MANUAL_MODEL` pin, even when the tenant's BYO provider happens to match the platform's active one.
- New `src/orbicrew_api/byo_keys.py`: `/v1/byo-keys` router (list/create/test/activate/deactivate/delete), copying `agents.py`'s exact `Depends(get_current_auth)` + `get_tenant_conn` pattern. Creating a tenant's very first BYO key auto-activates it; adding a second/third never silently switches an already-active key out from under a tenant relying on it. Activating an `is_valid: false` key is rejected (400) rather than silently no-op'ing (a previously-tested-invalid key would otherwise show "Active" in the UI while task execution quietly kept using the platform key).
- New `src/orbicrew_api/byo_provider.py`: `get_active_byo_key(pool, tenant_id)` — a small worker-side resolver (separate from the router since it's called from `worker.py`'s own superuser pool, trusted internal code, same pattern as the existing `get_tenant_spend_24h` fetch there).
- **Threaded `tenant_id`/BYO resolution through task execution for the first time** — previously entirely absent: `worker.py`'s `run_task_job` now resolves the tenant's active BYO key once (mirroring its existing `tenant_spend_24h_usd` pre-fetch) and passes `byo_provider`/`byo_api_key` into `run_office_manager(...)` → `OfficeManagerState` → `_make_specialist_node` → `llm_client.complete()`/`complete_research()`. The Research Agent's tool-use loop stays Anthropic-schema-specific (an existing Phase 1.7 decision) — its BYO override only ever applies when the tenant's active BYO provider is itself `anthropic`, filtered in `office_manager.py` before the call, not inside `llm_client.py`. No cross-provider fallback on a BYO call failure — a bad/revoked BYO key is a real failure, flows into the existing Phase 1.2 retry-then-escalate path, exactly like Phase 1.8's philosophy for the platform's own provider calls.
- **Tests**: new `tests/test_crypto.py`, `tests/test_byo_keys.py` (12 router cases). Extended `tests/test_model_providers.py` (`validate_provider_key`, `api_key_override`, `allow_manual_override=False`), `tests/test_llm_client.py` (BYO override for both the anthropic-native and OpenAI-compatible paths), `tests/test_office_manager.py` (BYO threading, research's anthropic-only filtering), `tests/test_worker.py` (BYO key passed through to `run_office_manager`; also fixed every existing test's `pool.fetchrow` mock, which previously used a single `return_value` for what is now two sequential calls — the task-row fetch and the new BYO-key lookup). 130 tests passing (was 106).

**Live verification** (real DB, real running dev API + a restarted arq worker — asked the user first via `AskUserQuestion` before killing their running worker process, same as 2.1's precedent for the API process): created a real BYO DeepInfra key for the founder dev tenant (auto-activated as the tenant's first key, validated ✓ live); a real submitted task's `model_used` came back `deepinfra/deepseek-ai/DeepSeek-V4-Flash`. Added a deliberately-wrong OpenAI key — came back `is_valid: false`, activating it correctly rejected with 400. Added a real OpenRouter key (chosen specifically because it differs from the platform's own `.env`-configured active provider, `deepinfra` — the earlier deepinfra-vs-deepinfra test hadn't actually proven the override took effect over the platform default, since both coincided) and activated it — the next task's `model_used` came back `openrouter/deepseek/deepseek-v4-flash`, proving the tenant override genuinely wins. Deactivated and confirmed the next task reverted to the platform's own `deepinfra` key. Cleaned up all test BYO keys from the founder tenant afterward.

**`repos/orbicrew-web`:**
- Regenerated `src/lib/api-schema.d.ts` from the live API. New `src/lib/api-byo-keys.ts` (typed wrapper, `Result`-union pattern matching `api-agents.ts`).
- New `/settings` page (`src/app/(dashboard)/settings/`) — the first real Settings sub-page. A left tab list shows `Connected Channels`/`Plan & Billing`/`Team Members`/`Interface Preference` disabled ("Coming soon", reusing `sidebar-nav.tsx`'s existing disabled-span convention) with only **API Keys** enabled: a list of configured providers (masked `sk-…{key_last4}`, Active/Valid badges, Test connection/Activate-or-Deactivate/Delete buttons via the `.bind(null, provider)` parameterized-action pattern from `approval-item.tsx`) plus an "Add a model provider" form (provider `<select>`, `type="password"` key input matching `login-form.tsx`'s existing convention). `sidebar-nav.tsx`'s `Settings` entry gets a real `href` for the first time.
- `npx tsc --noEmit`, `npm run lint`, and a full `npm run build` all clean (per the AGENTS.md warning here about training-data drift on this Next.js version, and 2.1's precedent of `next build` catching a bug `next dev` didn't — none found this time).
- **Live-verified with a real headless browser** (Playwright, installed into the scratchpad since this repo doesn't vendor it — same fallback pattern 2.1 used): logged in as the founder dev tenant, added a real DeepInfra key on `/settings` → masked key + Valid + Active badges rendered correctly → Test connection → Deactivate (Active badge disappears, Activate button returns) → Delete (empty state returns) — zero console/page errors throughout. Cleaned up the test key afterward (confirmed empty list via a direct API call).

### Decisions / assumptions
- No plan-based usage-cap enforcement on platform-key usage was built here — the user's own framing explicitly separates "BYO = lower price, tenant's own bill" from "platform key = capped by subscribed plan," but the latter needs real pricing tiers (Phase 2.2, not started) to enforce against. Left a forward pointer in the phase plan.
- `usage_records`/`task_steps.model_used` keep recording real cost/tokens for BYO calls too (tenant-facing transparency, and useful for a future "you've saved $X using your own key" surface) — no new billing/invoicing logic was added beyond that.
- JWTs/session handling, admin `/settings`, and everything else about 2.1's auth model are unchanged.

### Files / repos touched
- `repos/orbicrew-api`: `src/orbicrew_api/crypto.py`, `byo_provider.py`, `byo_keys.py` (all new), `model_providers.py`, `llm_client.py`, `office_manager.py`, `worker.py`, `main.py`, `settings.py`, `migrations/0008_byo_provider_keys.sql` (new), `pyproject.toml`/`uv.lock` (`cryptography`); `tests/test_crypto.py`, `test_byo_keys.py` (new), `test_model_providers.py`, `test_llm_client.py`, `test_office_manager.py`, `test_worker.py`
- `repos/orbicrew-web`: `src/lib/api-byo-keys.ts` (new), `api-schema.d.ts` (regenerated), `src/app/(dashboard)/settings/` (new: `page.tsx`, `actions.ts`, `byo-key-row.tsx`, `add-byo-key-form.tsx`), `src/components/sidebar-nav.tsx`
- `docs/development/`: `phase_by_phase_development_plan.md` (2.3 marked done), `manual-test-guide.md` (2.4 row filled in — the guide's pre-existing row number for BYO keys — plus changelog entry), `development-tracker.md` (this entry)
