...
| 1.8 | Multi-provider cost-tier routing | Replace `llm_client.py`'s Anthropic-only call with a small **in-app provider registry** (no self-hosted gateway/server — just an HTTP call to a different `base_url` per provider from the existing FastAPI service). **Tier→provider mapping refined 2026-08-25** (see tracker for the pricing research behind it): `mid`/`frontier` route **direct to Anthropic** — no third-party host prices Claude below Anthropic's own rate (DeepInfra and OpenRouter both pass it through at ~list price), so routing those tiers through the registry's aggregators would add a dependency for zero savings and lose first-party features like prompt caching. `trivial`/`cheap` use the registry: **`trivial`** tries **Groq then Cerebras** first — both permanent no-card free tiers, scoped to this tier only (Gemini's free tier excluded — Google uses free-tier content to improve its products) — falling through to **`cheap`** on any error; **`cheap`** uses **DeepInfra then OpenRouter** for open-weight models (DeepInfra typically cheapest per-model with no credit-purchase fee; OpenRouter's value there is its own internal multi-host fallback for the same model, used when DeepInfra is down or doesn't host it). **Auto mode (default)**: router walks this priority order per tier, with automatic fallback to the next on error/unavailability. **Designed to extend without a rewrite**: adding AWS Bedrock or a direct OpenAI key later is just a new registry entry, not an architecture change — this is deliberately the same shape as the BYO Pattern A/B/C design in `tech/13_byo_provider_architecture.md`, scoped down to single-user for now so Phase 2.3's eventual LiteLLM migration is an upgrade of this pattern, not a rewrite. Closes the gap `model_router.py`'s own comment already flagged ("self-hosted LiteLLM / OpenRouter routing is deferred") and reaffirms the original Phase 0 spec (§12 of `01_AI_Office_Platform_Requirements.md` always called for "OpenRouter-based model routing"). Coding-agent work (opencode/Claude Code) stays external to Orbicrew, pointed at whichever registered provider, rather than duplicated as an in-app specialist | Phase 0.4–0.5, 1.4a | api | **Done, live verification pending real keys (2026-08-25)** — new `src/orbicrew_api/model_providers.py`; all four providers are OpenAI-compatible at the chat-completions surface (unlike the four different search-provider JSON shapes), so one implementation (the existing `openai` SDK, base_url swapped) covers Groq/Cerebras/DeepInfra/OpenRouter. `llm_client.complete()` tries the registry first for `trivial`/`cheap` and falls back to direct Anthropic Haiku on any registry failure/no-key-configured, so the system works unchanged with zero new `.env` keys. **Manual per-agent override deferred**: `office_manager.py`'s specialist routing is still purely keyword-based with no linkage from a running task to a specific `agents` row, so `agents.model_mode`/`manual_model` (added in 1.4a) can't actually be honored yet — building a "manual override" now would be a shallow no-op wired to nothing real; revisit once task execution looks up a real agent row. 85 tests passing (9 new). See tracker for full detail and the model-ID sourcing (live-verified via WebFetch/WebSearch, not memory, given the Jan-2026 knowledge cutoff vs. today's date). |
...
20. Phase 2 planning: tenant model + RLS (2.1) is the next real engineering slice once the user decides to move past Phase 1 — not started yet, no code exists for it.
