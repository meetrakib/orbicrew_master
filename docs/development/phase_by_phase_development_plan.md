...
22. Phase 2.2 (usage records/tiers/billing UI, minus the deferred payment-processor wiring), 2.5 (tenant settings admin), and 2.7 (project isolation) are all real, un-started engineering slices that now depend on 2.1's real tenant/auth model — pick the next one when resuming. Note 2.2's future usage-cap enforcement on platform-key usage will want to read `byo_provider_keys.is_active` (2.3) to know which tenants are exempt (their spend is on their own account, not the platform's).
