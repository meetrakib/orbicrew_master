...
| 1.7 | Research agent + tools | Web search/fetch wired for real research | Phase 0 agents | api | **Done (code + unit tests; live run pending a real `TAVILY_API_KEY`)** — `src/orbicrew_api/research_tools.py` adds Anthropic tool-use schemas for `web_search`/`web_fetch` plus their implementations, behind a small `SearchProvider` abstraction (one registered provider today, Tavily, chosen the same way Phase 1.8's model-provider registry works — a new provider is a new class, not a rewrite; see tracker for the Tavily-vs-Brave-vs-SearXNG research that led here). `llm_client.py` gains `complete_research()` — a tool-use conversation loop (≤6 turns) used only by the `research` specialist node in `office_manager.py`; every tool call is logged as its own `tool_call` task step (via the existing `task_steps.tool_call` column, no schema change) before the final `specialist_execute` step, and a failed search/fetch is returned to the model as an `is_error` tool result rather than crashing the task. 12 new/updated tests (`test_research_tools.py`, `test_llm_client.py`, plus a research-path case in `test_office_manager.py`) |
...
18. ~~Phase 1.7: Research agent + tools — real web search/fetch.~~ **Done** (code + unit tests; live run pending a real `TAVILY_API_KEY`).
19. Phase 1.8: Multi-provider cost-tier routing — DeepInfra + OpenRouter registry (only remaining Phase 1 item).
