...
| 2.5a | Connected Channels — per-tenant self-service | Real per-tenant connect/disconnect flow for Telegram/Discord/WhatsApp/Slack from Settings, per `tech/10_ui_ux_guide.md` §12 — replacing the current model where each channel adapter is a **separate deployed process per tenant**, hand-configured via its own `.env` (`ORBICREW_API_KEY` + e.g. `TELEGRAM_ALLOWED_CHAT_ID`), with no self-service UI and no backend concept of a "channel connection" at all. Needs a new `tenant_channel_connections`-style table + a real linking flow per channel (e.g. a Telegram bot deep-link or chat-ID binding step, WhatsApp blocked on Meta business verification same as 1.6/2.2's Stripe blocker). **Explicitly flagged by the user (2026-08-25) as required for every tenant — not optional polish — but not yet scoped in implementation detail; deliberately deferred, to be picked up later.** | 2.1, channel adapters (1.5/1.6) | api, web, channels | Deferred — tracked here so it isn't lost, not yet started |
...
23. Phase 2.2 (usage records/tiers/billing UI, minus the deferred payment-processor wiring), 2.5a (Connected Channels — per-tenant self-service, **explicitly required by the user, not optional — deferred but must not be dropped**), and 2.7 (project isolation) are the remaining real, un-started engineering slices that depend on 2.1's real tenant/auth model — pick the next one when resuming. Note 2.2's future usage-cap enforcement on platform-key usage will want to read `byo_provider_keys.is_active` (2.3) to know which tenants are exempt (their spend is on their own account, not the platform's), and could reuse 2.5's `PLAN_CATALOG` shape for spend limits per tier.
