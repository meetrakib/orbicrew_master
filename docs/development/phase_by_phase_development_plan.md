...
21. Phase 2.2 (usage records/tiers/billing UI, minus the deferred payment-processor wiring), 2.3 (BYO key management), 2.5 (tenant settings admin), and 2.7 (project isolation) are all real, un-started engineering slices that now depend on 2.1's real tenant/auth model — pick the next one when resuming.
