...
| 4. Model routing & execution layer | Chooses cheapest sufficient model per task, calls it through a gateway (platform keys for managed tenants, any of three BYO patterns for BYO tenants), enforces caps/caching. **Phased**: Phase 0/1 uses a small in-app provider registry (DeepInfra + OpenRouter to start, extensible to Bedrock/direct-Anthropic/direct-OpenAI — no separate server, just outbound calls from the existing API), auto-cheapest by default with per-agent manual override; self-hosted LiteLLM (a dedicated gateway server) is introduced starting Phase 2.3 once per-tenant virtual-key/BYO routing is actually needed | In-app provider registry (Phase 0/1) → self-hosted LiteLLM (open source, from Phase 2) + custom router service; see `tech/13_byo_provider_architecture.md` |
...
- **Self-hosted LiteLLM gateway** (open source) sits between the Router and every model call, for both managed and BYO tenants — one OpenAI-compatible interface across 140+ providers, self-hosted so it doesn't trade one vendor lock-in for another. See `tech/13_byo_provider_architecture.md` for the full design. **Near-term (Phase 0/1, single-tenant personal use): a small in-app provider registry instead of a separate gateway server** — DeepInfra and OpenRouter registered to start (auto-cheapest per tier, per-agent manual override reusing `agents.model_mode`/`manual_model`), extensible to AWS Bedrock, a direct Anthropic key, or a direct OpenAI key as new registry entries whenever needed, with no architecture change. This mirrors the BYO Pattern A/B/C shape below one tenant early, so Phase 2.3's self-hosted-LiteLLM migration upgrades this pattern rather than replacing it.
...
- **Digest:** one consolidated report of what happened since you last checked — what finished, what's blocked, what needs a decision — with links to real artifacts, not raw logs. Not scoped to a fixed time of day (a global product with users in every timezone can't assume "morning"). Purely on-demand — no background scheduler: history is browsed by querying an arbitrary past range live, since the underlying data is already persisted forever. A custom per-user delivery time is deferred until a real notification channel exists to deliver it through. Full detail in `tech/03_system_design.md` §7.
...
- **Ask-vs-proceed clarification behavior, per agent (new):** users can control whether an agent pauses to ask a question and wait for a reply when it hits an ambiguous decision, or proceeds using its best judgment and states its assumption. Three modes per agent (ask only when it matters, always proceed, always ask), with unattended tasks automatically proceeding rather than stalling, and every assumption made while unattended visible in the Digest. Full detail in `tech/15_performance_training_and_settings.md` Section 5.
...
| **Tool**               | **Type**                      | **Role in our system**                                                                              |
|------------------------|-------------------------------|-----------------------------------------------------------------------------------------------------|
| LiteLLM                | Open source, self-hosted      | Unified gateway abstracting all model access (managed pooled keys + all three BYO patterns) behind one OpenAI-compatible interface; self-hosted to avoid vendor lock-in. Target architecture from Phase 2.3 on — see `tech/13_byo_provider_architecture.md`. |
| DeepInfra              | Paid aggregator (usage-based) | **Phase 0/1: one of two registered providers in the in-app registry** — owns its inference stack (no aggregator markup/card fee), competitively priced on Claude specifically and on DeepSeek-class open models |
| OpenRouter             | Paid aggregator (usage-based) | **Phase 0/1: the other registered provider** — broader catalog and automatic cross-provider fallback if DeepInfra is unavailable or a needed model isn't listed there. Also one of the providers LiteLLM routes to once it's stood up, and directly usable by BYO customers who bring their own OpenRouter account (Pattern B) |
| AWS Bedrock            | Paid, usage-based             | Candidate future registry entry (Phase 1.8+) or BYO pattern — hosts Claude at Anthropic's direct list price plus DeepSeek/MiniMax, but at a per-token premium on the open models versus DeepInfra/OpenRouter; not registered by default today, worth revisiting if AWS-account consolidation or Claude-heavy volume makes it attractive |
...
- Budget guard + action whitelist + Digest.
