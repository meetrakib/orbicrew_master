...
## 0. Phased rollout (added 2026-08-25, updated 2026-08-25) — this document describes the Phase 2.3+ target architecture; Phase 1.8's in-app registry runs a lighter first increment of the same idea

Everything below describes the full self-hosted-gateway architecture for when there are real BYO customers to serve — a separate LiteLLM *server* process, per-tenant virtual keys, and enforced multi-tenant budgets. **That server doesn't exist yet.** Instead, `model_providers.py`/`llm_client.py` implement a small **in-app provider registry** (Phase 1.8, redesigned the same day it first shipped — see `phase_by_phase_development_plan.md` and the tracker) — the same conceptual shape as Patterns A/B/C below (a provider backend behind one router, chosen manually), just running as code inside the existing FastAPI service instead of a separate proxy. It's **not** a per-tier auto-fallback chain (an earlier version of this section described that design; it was replaced before shipping) — a single global switch, `MODEL_PROVIDER` (`anthropic` | `openai` | `openrouter` | `deepinfra`), picks one provider that serves **every** tier from its own model catalog, since the real driver turned out to be "spend the credits I hold in a specific account," not cost-arbitrage across providers. `MODEL_PROVIDER_MANUAL_MODEL` optionally pins one exact model for every tier on the active provider. Groq and Cerebras were evaluated (permanent no-card free tiers) but excluded — neither has a mid/frontier-class model, so neither can serve "every tier." The registry is designed to grow by adding entries — **AWS Bedrock** — without an architecture change, matching this document's Pattern A (direct key) and Pattern B (OpenRouter) shapes, one active provider at a time.

**Why not the full gateway yet:** the multi-tenant virtual-key, per-tenant budget-cap, and cross-BYO-pattern enforcement described below are a distinct requirement from "which provider serves model calls" — Phase 2.1 (`phase_by_phase_development_plan.md`) landed real multi-tenancy (tenant signup/auth, Postgres RLS) separately from BYO key management, so real tenants exist now but each still shares the platform's single active `MODEL_PROVIDER` rather than bringing their own key. Running a separate LiteLLM instance before that's a real requirement would be infrastructure to operate for no benefit over the in-app registry.

**Self-hosted LiteLLM is introduced at Phase 2.3** (`phase_by_phase_development_plan.md`), when Pattern A/B/C BYO routing and per-tenant enforcement in Section 2 below become real requirements — each tenant bringing/choosing their own provider key, not just the platform-wide `MODEL_PROVIDER` switch. Because Phase 1.8's provider registry already mirrors this document's shape (one router, swappable backends), that migration is an upgrade of an existing pattern (swap the in-app registry for LiteLLM's proxy, keep the same provider concepts, add the per-tenant dimension), not a rewrite. Until Phase 2.3, treat this document as the design to build *toward* for per-tenant BYO specifically — platform-wide provider flexibility already exists via Phase 1.8, and real tenant isolation already exists via Phase 2.1.
